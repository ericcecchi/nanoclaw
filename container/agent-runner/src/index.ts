    ...
    mcpServers: {
      nanoclaw: {
        ...
        env: {
          ...
          NANOCLAW_TRIGGER_MESSAGE_ID: containerInput.triggerMessageId || '',
        },
        tools: ['*'],
      },
    },
...
async function main(): Promise<void> {
  ...
  const minimalEnv: Record<string, string> = {
    ...
    LANG: 'C.UTF-8',
    // Pass GitHub token so gh CLI works in bash tool calls
    GH_TOKEN: githubToken,
    GITHUB_TOKEN: githubToken,
    // Point kubectl at the mounted kubeconfig (if present)
    KUBECONFIG: '/workspace/extra/kube/config',
    // Research tool API keys (passed through if present)
    ...(secrets.TAVILY_API_KEY ? { TAVILY_API_KEY: secrets.TAVILY_API_KEY } : {}),
    ...(secrets.INFSH_API_KEY ? { INFSH_API_KEY: secrets.INFSH_API_KEY } : {}),
  };
